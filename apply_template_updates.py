...
<title>Login - Gruha Alankara</title>
...
h1{text-align:center;font-size:1.8rem;font-weight:700;margin-bottom:0.5rem;background:linear-gradient(135deg,#818cf8,#a855f7);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;}
...
input[type=email],input[type=password],input[type=text]{width:100%;background:#0d0d1a;border:1px solid #2d3748;color:#e2e8f0;border-radius:8px;padding:0.75rem 0.75rem 0.75rem 2.5rem;font-size:0.95rem;transition:border-color 0.2s;}
input:focus{outline:none;border-color:#7c3aed;box-shadow:0 0 0 3px rgba(124,58,237,0.1);}
.btn-submit{width:100%;padding:0.85rem;background:linear-gradient(135deg,#7c3aed,#a855f7);color:white;border:none;border-radius:8px;font-size:1rem;font-weight:600;cursor:pointer;margin-top:0.5rem;transition:all 0.2s;display:flex;align-items:center;justify-content:center;gap:8px;}
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Register - Gruha Alankara</title>
<link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css" rel="stylesheet">
<style>
*{margin:0;padding:0;box-sizing:border-box;}
body{background:#0d0d1a;color:#e2e8f0;font-family:"Segoe UI",sans-serif;min-height:100vh;}
...
index_content = index_content.replace(
    "setTimeo=>window.location.href='/design?style='+encodeURIComponent(name),600);",
    "setTimeout(()=>window.location.href='/design?style='+encodeURIComponent(name),600);"
)

with open(index_path, 'w', encoding='utf-8') as f:
    f.write(index_content)

print("login.html written to:", login_path)
print("register.html written to:", register_path)
print("index.html JS fixed")
print("All done - restart Flask to see changes")
